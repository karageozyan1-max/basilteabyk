...
  // Treat "packSize" as how many bottles were added at once (from Shop),
  // and "quantity" as the cart line quantity (adjusted via +/- in the cart).
  const lineQty = (item: any) => (item.packSize ?? 1) * (item.quantity ?? 1);
  const lineTotal = (item: any) => (item.price ?? 0) * lineQty(item);
...
                    {/* e.g. "3 × 16oz Basil Tea with Honey" */}
                    {lineQty(item)} × {item.size} {item.name}
                  </Text>
...
                {/* Quantity controls (updates the line's "quantity") */}
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  <TouchableOpacity
                    onPress={() => updateQuantity(item.id, Math.max(0, (item.quantity ?? 1) - 1))}
